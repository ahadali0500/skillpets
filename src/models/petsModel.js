...
module.exports.updateHappiness = (pet_id, newHappiness, callback) => {
    const SQLSTATEMENT = `
        UPDATE userpets
        SET happiness_level = happiness_level + ?
        WHERE pet_id = ?;
    `;
    const VALUES = [newHappiness, pet_id]; // Corrected order of parameters
    pool.query(SQLSTATEMENT, VALUES, callback);
};
...
module.exports.getPetsByUserId = (user_id, callback) => {
    const SQLSTATEMENT = `
        SELECT userpets.pet_id, pets.pet_name
        FROM userpets
        JOIN pets ON userpets.pet_id = pets.pet_id
