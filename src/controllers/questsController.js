...
module.exports.createQuest = (req, res, next) => {
    ...
    if (!data.quest || !data.reward) {
        return res.status(400).json({
            message: "Missing required fields. Please include quest, and reward.",
        });
    }
...
module.exports.getUserQuests = (req, res, next) => {
    ...
    model.getQuestsByUserId(null, callback);
};
...
module.exports.updateQuestStatusWithReward = (req, res) => {
    const { user_id } = req.params; // User ID from URL params
    const { quest_id, pet_id, reward } = req.body; // Quest ID and new status from the request body

    const rewardPoints = reward; // Fetch reward points from the quest
    ...
    const updateSkillPointsCallback = (error) => {
        ...
        model.updateQuestStatus({ quest_id, user_id, pet_id, status}, updateQuestStatusCallback);
    };
